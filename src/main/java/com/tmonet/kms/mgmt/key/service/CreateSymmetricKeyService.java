...
@Service
public class CreateSymmetricKeyService {

//	private static final Logger logger = LoggerFactory.getLogger(CreateSymmetricKeyService.class);
	...
	public void storeSymmetricKeyInfo(String profileId, String keyId, HsmServiceInfoVo hsmGroupInfo,
		...
		try {
			...
			// NKM_KEY_ATTR_LIST 테이블에 키 속성 데이터 저장
//			for (KeyProfileAttrListVo keyProfileAttr : keyProfileAttrList) {
//				KeyAttrListVo keyAttrList = new KeyAttrListVo();
//				keyAttrList.setKEY_ID(keyId);
//				keyAttrList.setPARTITION_ID(hsmGroupInfo.getPARTITION_ID());
//				keyAttrList.setGROUP_ID(hsmGroupInfo.getGROUP_ID());
//				keyAttrList.setATTR_ID(keyProfileAttr.getATTR_ID());
//				keyAttrList.setVALUE(keyProfileAttr.getATTR_DEFAULT());
//				keyAttrList.setREG_USER("MGMT");
//				if (mapper.insertKeyAttrList(keyAttrList) < 1) {
//					throw new KMSException(KMSErrorCode.DATABASE_INSERT_FAILURE);
//				}
//			}
			...
			keyHistory.setTYPE(EnumOperationCode.OP_CODE_CREATE.getKey());
			keyHistory.setPARAM("");
			keyHistory.setHMAC("");
