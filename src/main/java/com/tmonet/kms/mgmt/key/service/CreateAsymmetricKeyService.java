...
@Service
public class CreateAsymmetricKeyService {

//	private static final Logger logger = LoggerFactory.getLogger(CreateAsymmetricKeyService.class);
	...
	public HsmCreateAsymmetricKeyRequest makeHsmCreateAsymmetricKeyRequestData(String partitionId,
			List<KeyProfileAttrListVo> priKeyProfileAttrList, List<KeyProfileAttrListVo> pubKeyProfileAttrList) {
		try {
			HsmCreateAsymmetricKeyRequest reqData = new HsmCreateAsymmetricKeyRequest();
			ApiInfo apiInfo = new ApiInfo();
			apiInfo.setApiId("HSM.ASK.REG.01");
			...
			for (KeyProfileAttrListVo vo : priKeyProfileAttrList) {
				...
				case "keyLength": // 키 길이
					...
					break;
				case "publicExponent":
					reqData.setPublicExponent(vo.getATTR_DEFAULT());
					break;
				case "curve":
					reqData.setCurve(vo.getATTR_DEFAULT());
					break;
				}
			}
	...
	public void storeAsymmetricKeyInfo(String profileId, String keyId, HsmServiceInfoVo hsmServiceInfo,
			List<KeyAttrListVo> keyAttrList) {
		try {
			// NKM_KEY_LIST 테이블에 키 데이터 저장
			KeyListVo keyList = new KeyListVo();
			keyList.setKEY_ID(keyId);
			keyList.setPARTITION_ID(hsmServiceInfo.getPARTITION_ID());
			keyList.setGROUP_ID(hsmServiceInfo.getGROUP_ID());
			...
			for (KeyAttrListVo keyAttr : keyAttrList) {
				...
				keyAttr.setPARTITION_ID(hsmServiceInfo.getPARTITION_ID());
				keyAttr.setGROUP_ID(hsmServiceInfo.getGROUP_ID());
			...
			keyHistory.setPARTITION_ID(hsmServiceInfo.getPARTITION_ID());
			keyHistory.setGROUP_ID(hsmServiceInfo.getGROUP_ID());
			keyHistory.setTYPE(EnumOperationCode.OP_CODE_CREATE_KEY_PAIR.getKey());
			keyHistory.setPARAM("");
			keyHistory.setHMAC("");
