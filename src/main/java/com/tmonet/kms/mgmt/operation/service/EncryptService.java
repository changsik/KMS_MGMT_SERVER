...
//	private static final Logger logger = LoggerFactory.getLogger(EncryptService.class);
	...
	public HsmEncryptRequest makeHsmEncryptRequestData(EncryptRequest request, String partitionId) {
		try {
			HsmEncryptRequest reqData = new HsmEncryptRequest();
			ApiInfo apiInfo = new ApiInfo();
			apiInfo.setApiId("HSM.OP.ENC.01");
	...
	public void storeKeyInfo(String keyId, HsmServiceInfoVo hsmGroupInfo) {
		try {
			...
			keyHistory.setTYPE(EnumOperationCode.OP_CODE_ENCRYPT.getKey());
			keyHistory.setPARAM("");
			keyHistory.setHMAC("");
