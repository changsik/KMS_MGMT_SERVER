...
//	private static final Logger logger = LoggerFactory.getLogger(DecryptService.class);
	...
	public HsmDecryptRequest makeHsmDecryptRequestData(DecryptRequest request, String partitionId) {
		try {
			HsmDecryptRequest reqData = new HsmDecryptRequest();
			ApiInfo apiInfo = new ApiInfo();
			apiInfo.setApiId("HSM.OP.DEC.01");
			...
			reqData.setCiphertext(request.getCiphertext());
	...
	public void storeKeyInfo(String keyId, HsmServiceInfoVo hsmGroupInfo) {
		try {
			...
			keyHistory.setTYPE(EnumOperationCode.OP_CODE_DECRYPT.getKey());
			keyHistory.setPARAM("");
			keyHistory.setHMAC("");
