...
//	private static final Logger logger = LoggerFactory.getLogger(VerifyService.class);
	...
	public HsmVerifyRequest makeHsmVerifyRequestData(VerifyRequest request, String partitionId) {
		try {
			HsmVerifyRequest reqData = new HsmVerifyRequest();
			ApiInfo apiInfo = new ApiInfo();
			apiInfo.setApiId("HSM.OP.VERIFY.01");
			...
			reqData.setSignature(request.getSignature());
	...
	public void storeKeyInfo(String keyId, HsmServiceInfoVo hsmGroupInfo) {
		try {
			...
			keyHistory.setTYPE(EnumOperationCode.OP_CODE_SIGNATURE_VERIFY.getKey());
			keyHistory.setPARAM("");
			keyHistory.setHMAC("");
