...
import com.tmonet.kms.mgmt.hsmgroup.service.UpdateHsmGroupService;

@RestController
@RequestMapping("/kms")
public class UpdateHsmGroupController {

//	private static final Logger logger = LoggerFactory.getLogger(UpdateHsmGroupController.class);

	@Autowired
	private UpdateHsmGroupService service;

	@PatchMapping("/hsm/group/{groupId}")
	public @ResponseBody HsmGroupResponse updateHsmGroup(@PathVariable String groupId,
			@RequestBody @Validated HsmGroupRequest request, Errors errors) {
		...
		// 2. HSM 그룹 정보 수정
		service.updateHsmGroupInfo(request, groupId);
